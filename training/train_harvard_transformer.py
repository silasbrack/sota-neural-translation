...
criterion = LabelSmoothing(size=OUTPUT_DIM, padding_idx=pad_idx, smoothing=0.1).to(device)
BATCH_SIZE = 64
...
if TO_TRAIN:
    ...
    for epoch in range(NUM_EPOCHS):
        train(train_iter, model, criterion, model_opt)
        loss = eval(valid_iter, model, criterion)
        print(loss)

        checkpoint_and_save('harvard_transformer2.pt', model)
else:
    model.load_state_dict(torch.load("harvard_transformer.pt"))

    loss = eval(test_iter, model, criterion)
    print(loss)
